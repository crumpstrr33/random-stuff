...
def add_cyph_nums(cyph_nums):
    ...
    new_num = "".join(
        map(
            lambda x: str((int(x) - 1) % len(CYPHER) + 1), str(sum(map(int, cyph_nums)))
        )
    )
...
def calc_every_comb(*words, wta, keep_cyphers, duplicate, check_match):
    ...
    fdict = {
        "w": max_len_w if max_len_w > 8 else 8,
    ...
    row = "|" + wta * "{:^{w}}|" + "{:^{n}}|" + keep_cyphers * (wta + 1) * "{:^{c}}|"
    row_titles = (
        ["Word " + str(x + 1) for x in range(wta)]
        + ["New Word"]
        + keep_cyphers * (["Cypher " + str(x + 1) for x in range(wta)] + ["Sum"])
    )
    ...
    print(
        row_bar.format(
            *["-" * fdict["w"]] * wta, "-" * fdict["n"], *["-" * fdict["c"]] * (wta + 1)
        )
    )
    for n, word_comb in enumerate(word_combs):
        ...
        print(
            row.format(
                *word_comb, new_words[n], *cyph_nums_list[n], sum_nums_list[n], **fdict
            )
        )
    print(row_ext)
...
def input_words_main():
    ...
    parser = ArgumentParser(
        description="Cypher words and return the word " + "created by their sum."
    )
    parser.add_argument(
        "-f",
        "--file",
        help="Text file with space-separated "
    ...
    parser.add_argument(
        "-c",
        action="store_true",
        help="Will print the " + "cypher numbers along with the words",
    )
    parser.add_argument(
        "-d",
        action="store_true",
        help="Will allow for " + "duplicates of words to be summed",
    )
    parser.add_argument(
        "-m",
        action="store_true",
    ...
    parser.add_argument(
        "--print-cypher",
        action="store_true",
        help="Will " + "print out the cypher being used",
    )
    ...
    if args.print_cypher:
        ...
        if not args.d and len(args.words) < int(args.num or 2):
            raise Exception(
                "The number of words must be at least equal to the "
